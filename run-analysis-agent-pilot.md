...
- **Does not depend at runtime on**: any other document, including
  `docs/run-artifact-contract.md`. The pilot's input allow-list is frozen
  in this spec (see §2.2).
- **Agent id**: `run-analysis-agent`
- **Variant**: Analyst — extractive (no LLM, no stochastic components).
- **Supersedes**: `run-analysis-agent-contract-pilot.md` (draft).
...
### 2.2 Allow-list of supported inputs

The pilot reads **only** the files below. Files outside this allow-list
are reported as *unexpected extras* (warning, not fatal — see §5). The
pilot does not recurse into subdirectories.

| Role                | Filename        | Required | Format | Notes                                                |
|---------------------|-----------------|----------|--------|------------------------------------------------------|
| Run metadata        | `metadata.json` | required | JSON   | Run identifier, timestamps, environment, parameters. |
| Performance summary | `metrics.json`  | required | JSON   | Aggregated metrics of the run.                       |
| Run report          | `report.json`   | required | JSON   | Structured report artifact of the run.               |
| Config snapshot     | `config.json`   | optional | JSON   | Configuration as used by the run.                    |

**Required files (exactly three):** `metadata.json`, `metrics.json`,
`report.json`.
**Optional files (one):** `config.json`.
...
- All three required files of §2.2 are present and non-empty.

### 2.4 Non-preconditions (explicitly tolerated)

- `config.json` may be absent; its absence is reported, not fatal.
- Files present beyond the allow-list are reported as unexpected extras,
  not fatal.
- Subdirectories under the base path are ignored and reported as
  unexpected extras.
- File sizes are not bounded by this contract. The pilot may apply its own
  read limits per file and must report truncation in the anomalies
  section.
...
- `docs/run-artifact-contract.md` (no runtime dependency; the allow-list
  is frozen in §2.2)
...
- If the target markdown already exists, the agent **must not** overwrite
  it silently. Behavior is one of:
...
2. **Input summary**
   - Allow-listed files found, by role (from the §2.2 table).
   - Required files missing (explicit list; empty list is the normal case
     and must be shown as such — when non-empty, the agent would have
     aborted under §5).
   - Optional files missing (empty list or a single entry for
     `config.json`).
...
4. **Anomalies observable from artifacts**
   - Empty files, truncated files, timestamps out of order, duplicated
     entries within a file, encoding issues.
   - Each anomaly has a severity (`info`, `warn`, `error`) and a pointer.
5. **Open questions**
   - Things the pilot cannot determine from the allow-list alone and that
     a human or a different agent would need to answer.
6. **Provenance**
...
non-extractive variant): `model_id`, `prompt_version`, sampling
parameters.
...
| `E_REQUIRED_FILES_MISSING`    | Any of `metadata.json`, `metrics.json`, `report.json` is absent or empty.                 | No              | Yes          |
...
| `W_OPTIONAL_FILES_MISSING`    | `config.json` missing. Not fatal; surfaced in the report.                                 | Yes             | Yes          |
| `W_UNEXPECTED_EXTRAS`         | Files or subdirectories in input not declared by the allow-list. Not fatal.               | Yes             | Yes          |
...
- **I1 — Read allow-list**: the set of paths actually read is a subset of
  the union of §2.2 (four files under the input base path) and §2.5
  (external reads). Reads of `execution/`, `brokers/`, `pretrade/`,
...
- **I3 — Deterministic output**: given the same input (`input_hash`
  ...
  the pilot produces byte-equivalent output under a declared
  normalization (line endings, trailing whitespace, timestamp field
  replaced by a stable marker during test).
- **I4 — Non-deletion**: the agent never deletes any file.
- **I5 — Non-mutation of inputs**: no file inside
  `outputs/runs/<run_id>/` is modified, touched (mtime), or renamed.
- **I6 — Bounded output**: the output report is a single markdown file;
  the output log is a single JSON file. No additional artifacts.
...
- **I8 — Version honesty**: `agent_version` and `spec_version` reflect
  what was actually loaded, not constants.
- **I9 — Failure before partial**: on any `E_*`, no partial report is
  left behind. The log is the only artifact on failure.
...
- **I12 — No runtime contract discovery**: the pilot does not read any
  contract document at runtime to determine its allow-list. The
  allow-list of §2.2 is frozen in this spec.
...
- Runtime reading of `docs/run-artifact-contract.md` or any other
  contract document.
- Support for any input file beyond the four listed in §2.2. In
  particular, this pilot does **not** support: run logs, equity or PnL
  series, trade or order records, and the `attachments/` subdirectory.
  Any of these may be added only by a superseding spec that revisits
  §2.2 and the error model.
